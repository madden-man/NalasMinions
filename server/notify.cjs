...
// Pass `at` (a Unix timestamp in seconds) to schedule the push for later via
// ntfy's delayed delivery — used for chore reminders. Configure per-deploy:
...
// Returns the published message's id when ntfy reports one (null otherwise).

async function notify({ source = "Nala's Minions", event = 'bump', message, title, at } = {}) {
  ...
  const headers = { Title: notifyTitle, Tags: 'bell', Priority: 'default' }
  if (at) headers.At = String(at) // ntfy scheduled delivery (Unix seconds)
