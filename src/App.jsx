...
// a delete action. The checkbox is the only control that completes a chore;
// tapping the rest of the row opens it for editing. Shared by the single-column
// lists and the monthly day list.
function TaskRow({ task, onToggle, onRemove, onEdit }) {
  return (
    ...
    >
      <ListItemButton onClick={() => onEdit(task)} sx={{ py: 1.25 }}>
        <ListItemIcon sx={{ minWidth: 44 }}>
          <Checkbox
            edge="start"
            checked={task.done}
            // Toggle completion only from the checkbox; stop the click from
            // bubbling up to the row (which opens the edit dialog).
            onClick={(e) => e.stopPropagation()}
            onChange={() => onToggle(task.id)}
            inputProps={{ 'aria-label': `Mark ${task.text} done` }}
          />
        </ListItemIcon>
        <ListItemText
          primary={task.text}
...
function ChoresPage({ navigate }) {
  ...
  // Open the dialog to add a new chore, or (with a task) to edit an existing one.
  const openAddDialog = () => {
    setEditingTask(null)
    setDialogOpen(true)
  }
      ...
      <List disablePadding>
        {visibleTasks.length === 0 ? (
          ...
          visibleTasks.map((task) => (
            <TaskRow key={task.id} task={task} onToggle={toggle} onRemove={remove} onEdit={openEditDialog} />
          ))
        )}
      </List>
    </Paper>
